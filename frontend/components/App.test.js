...
//TEXT_FAV_LANG
test(`LABEL_FAV_LANG is visible`, () => {
  render(<App lang="en" />)
  expect(screen.getByLabelText(txt.en.LABEL_FAV_LANG)).toBeVisible()
})
...
test(`PLACEHOLDER_USERNAME is visible`, () => {
  ...
  test(`LABEL_FAV_LANG is visible`, () => {
    render(<App lang="esp" />)
    expect(screen.getByText(txt.esp.LABEL_FAV_LANG)).toBeVisible()
  })
      test(`LABEL_USERNAME is visible`, () => {
        render(<App lang="esp" />)
